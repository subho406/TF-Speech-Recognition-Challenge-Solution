...
class AudioProcessor(object):
  ...
  def __init__(self, data_dir, silence_percentage, unknown_percentage,
               ...
               model_settings,use_silence_folder=False,use_spectrogram=False):
    self.use_silence_folder=use_silence_folder
  ...
  def prepare_data_index(self, silence_percentage, unknown_percentage,
    ...
    wanted_words_index = {}
    if self.use_silence_folder==True:
    ...
    for set_index in ['validation', 'testing', 'training']:
      ...
      if self.use_silence_folder==False:
  ...
  def get_data(self, how_many, offset, model_settings, background_frequency,
    ...
    for i in xrange(offset, offset + sample_count):
      ...
      if sample['label'] == SILENCE_LABEL and self.use_silence_folder==False:
        input_dict[self.foreground_volume_placeholder_] = 0
      else:
        input_dict[self.foreground_volume_placeholder_] = 1
      # Run the graph to produce the output audio.
      if self.use_spectrogram == True:
          data[i - offset, :] = sess.run(self.spectrogram, feed_dict=input_dict).flatten()
      else:
  ...
  def get_data_raw(self, how_many, offset, model_settings, background_frequency,
               background_volume_range, time_shift, mode, sess):
    """Gather samples from the data set, applying transformations as needed.
    ...
    for i in xrange(offset, offset + sample_count):
      ...
      if sample['label'] == SILENCE_LABEL and self.use_silence_folder==False:
        ...
        input_dict[self.foreground_volume_placeholder_] = 1
      # Run the graph to produce the output audio.
      data[i - offset, :] = sess.run(self.raw_processed, feed_dict=input_dict).flatten()
      label_index = self.word_to_index[sample['label']]
      labels[i - offset, label_index] = 1
    return data, labels
